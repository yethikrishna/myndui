...
<Example
  ...
  code={`import { ParticleDissolve } from "@/components/myndui/particle-dissolve";

export function Hero() {
  return <ParticleDissolve text="Myndui" mode="loop" trigger="in-view" className="text-primary" />;
}`}
>
...
```tsx
import { ParticleDissolve } from "@/components/myndui/particle-dissolve";
```
